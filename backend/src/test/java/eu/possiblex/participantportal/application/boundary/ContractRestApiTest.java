...
class ContractRestApiTest {
    ...
    void shouldReturnMessageOnGetContractAgreements() throws Exception {
        ...
        this.mockMvc.perform(get("/contract/agreement")).andDo(print()).andExpect(status().isOk())
            ...
            .andExpect(jsonPath("$[0].asset.id").value(ContractServiceMock.FAKE_ID_ASSET))
            .andExpect(jsonPath("$[0].asset.name").value(ContractServiceMock.NAME))
            .andExpect(jsonPath("$[0].asset.description").value(ContractServiceMock.DESCRIPTION))
